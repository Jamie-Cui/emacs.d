...
For 'emacs' skill, OPERATION can be:
  ...
  (let ((skill (magent-skills-get skill-name)))
    ...
    (pcase skill-name
      ("emacs"
       (magent-tools--invoke-emacs-skill operation args skill))
      (_
       (format "Error: skill '%s' is not yet implemented" skill-name)))))

(defun magent-tools--invoke-emacs-skill (operation args skill)
  "Invoke emacs skill OPERATION with ARGS using SKILL struct."
  (let* ((skill-dir (magent-skill-dir skill))
         (impl-file (expand-file-name "agent-skills-emacs.el" skill-dir)))

    (unless (file-exists-p impl-file)
      (cl-return-from magent-tools--invoke-emacs-skill
        (format "Error: implementation file not found: %s" impl-file)))

    ;; Build elisp expression based on operation
    (let ((elisp-expr
           (pcase operation
             ("list-functions"
              (if (null args)
                  "(error \"list-functions requires prefix argument\")"
                (format "(agent-skills/list-functions %S)" (car args))))
             ("describe-function"
              (if (null args)
                  "(error \"describe-function requires function-name argument\")"
                (format "(agent-skills/describe-function %S)" (car args))))
             ("eval-expression"
              (if (null args)
                  "(error \"eval-expression requires expression argument\")"
                (format "(agent-skills/eval-expression %S)" (car args))))
             ("execute-keys"
              (if (null args)
                  "(error \"execute-keys requires keys argument\")"
                (format "(agent-skills/execute-keys %S)" (car args))))
             ("minibuffer-prompt"
              "(agent-skills/minibuffer-prompt)")
             ("current-buffer-state"
              "(agent-skills/current-buffer-state)")
             (_
              (format "(error \"Unknown operation: %s\")" operation)))))

      ;; Load implementation and execute via emacsclient
      (let ((full-expr (format "(progn (load %S nil t) %s)"
                               impl-file elisp-expr)))
        (condition-case err
            (let ((result (shell-command-to-string
                           (format "emacsclient --eval %S" full-expr))))
              ;; Trim and return result
              (string-trim result))
          (error
           (format "Error executing skill: %s" (error-message-string err))))))))
