...
# (opt) configure gui emacs with tiling window manager
./configure --with-native-compilation --with-tree-sitter --with-sqlite3 --with-rsvg --without-x-toolkit --without-pop --without-mailutils
# (TODO) MacOS?
./configure --with-native-compilation --with-tree-sitter --with-sqlite3 --with-rsvg --without-pop --without-mailutils
# compile
sudo make install
#+end_src
