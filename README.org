...
- texlive: for latex
- direnv: for separating project envs (~sudo dnf install direnv~)

#+begin_src bash
# optional, if you have not done so, in ~/.zshrc
export LD_LIBRARY_PATH=$LD_LIBRARY_PATH:/opt/homebrew/lib/
brew install autoconf texinfo libgccjit gcc gnutls tree-sitter pkg-config # macos
#+end_src
